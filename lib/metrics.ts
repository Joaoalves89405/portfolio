...
export async function getGithubUserData() {
	...
	const { data: starredData } = await octokit.request('GET /users/:username/starred', {
		username: 'Joaoalves89405',
		per_page: 100, // limit to 100 results per page
	})
	...
	const userDataWithReposAndStars = {
		...
		repos: reposData.map((repo: { name: any; description: any; html_url: any }) => ({
			name: repo.name,
			description: repo.description,
			html_url: repo.html_url,
		})),
		starred: starredData.map((repo: { name: any; description: any; html_url: any }) => ({
			name: repo.name,
			description: repo.description,
			html_url: repo.html_url,
		})),
